...
if ($error==0)
{
	...
	$filename_=~s/\.bed$//;
	open (OUT,">$filename_vcf_somatic.fa-snv-wt.fasta");
    open (OUT_MOD,">$filename_vcf_somatic.fa-snv-mut.fasta");
    open (OUT_FS,">$filename_vcf_somatic.fa-snv-fs.fasta");
    open (LOG,">$filename_vcf_somatic.fa-snv-wt.log");
    open (LOG_MOD,">$filename_vcf_somatic.fa-snv-mut.log");
    open (STAT,">$filename_vcf_somatic.fa-snv-mod.stat");
	my $proteins_count=0;
	...
	foreach my $chr (sort keys %chr)
	{
		...
		#if (open (IN,"$dir/$chr.fa"))
		{
			...
			#					print LOG_MOD qq!$start+$segment_start,$segment_length: $seq__\n!;
						...
						for(my $n=0;$n<length($seq_original) && $stop_found==0;$n=$n+3)
                        {
                            ...
                            if (length($triplet)==3)
                            {
                                if ($mapping{$triplet}=~/\*/)
                                {
                                    ...
                                    if($n+3>$lenb_added+1)
                                    {
                                        print LOG qq!Error: No codon found in the wild type sequence:$name \n$protein_original\n!;
                                        $protein_original="";
                                    }
                                }
