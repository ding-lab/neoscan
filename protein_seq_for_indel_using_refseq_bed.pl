...
if ($error==0)
{
  ...
  my $filename_=$filename;
    ...
    open (OUT,">$filename_vcf_somatic.proteome-indel-wt.fasta");
    open (OUT_MOD,">$filename_vcf_somatic.proteome-indel-mut.fasta");
    #open (OUT_FS,">$filename-frame-shift.fasta");
    open (LOG,">$filename_vcf_somatic.proteome-indel-wt.log");
    open (LOG_MOD,">$filename_vcf_somatic.proteome-indel-mut.log");
    open (STAT,">$filename_vcf_somatic.proteome-indel.stat");
			...
			if ($line=~/^>$chr\s*/ or $line=~/^>$chr_\s*/)
			{
				...
				while ($temp=~s/^#([^#]+)#//)
				{
					...
					if ($bed{$name}=~/^([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)$/)
					{
                        ...
                        #$seq_original=$seq_added.$seq;
                        #}
                        #else {
                        #my $seq_added=substr($sequence,$end,10000);
                        #$seq_original=$seq.$seq_added;
                        #}
							  ...
							  #print $chr,"\t",$var_pos,"\n"; 
								...
								if($strand=~/\-/) { 
										    ...
										    if($vcf_new{"$chr#$var_pos"} eq "-") 
												...
												if($inframe==1) { $description_.="(DEL:".$chr."-".$var_pos."-".$leno."-".$vcf_type{"$chr#$var_pos"}.":".$int_3s."in_frame_del";} 
												else { $description_.="(DEL:".$chr."-".$var_pos."-".$leno."-".$vcf_type{"$chr#$var_pos"}.":".$int_3s."fs"; }
												}
											        ...
											        my $rev_pos=length($seq_original)-$i-$leno;
                                                                                                ...
                                                                                                if($inframe==1) { $description_.="(INS:".$chr."-".$var_pos."-".$lenn."-".$vcf_type{"$chr#$var_pos"}.":".$int_3s."-".$int_3e."in_frame_ins";}
                                                                                                else { $description_.="(INS:".$chr."-".$var_pos."-".$lenn."-".$vcf_type{"$chr#$var_pos"}.":".$int_3s."fs"; }
											  }
						...
						#	}
						 ...
						 $protein=~s/^([^\*]+)\*.*$//;
					         ...
					         if(($protein ne $protein_original) && ($num_indel==1))
							  ...
							  if (length($protein)>6 and $protein!~/^\*/  && length($protein_original)>6)
							   {
								print OUT qq!>$name-wt (MAP:$chr:$start$strand $segment_lengths $segment_starts)\n$protein_original\n!;
								$description_.=")";
								#print LOG_MOD qq!>$name-indel $descriptions{$name} (MAP:$chr:$start$strand $segment_lengths $segment_starts) $description_\n$protein\n!;
								print OUT_MOD qq!>$name-indel (MAP:$chr:$start$strand $segment_lengths $segment_starts) $description_\n$protein\n!;
							   }
